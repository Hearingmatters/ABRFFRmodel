...
#function to intitialize all the parameters        
    ...
    def solve(self):
        ...
        self.Vsolution=np.zeros([len(self.x),length+2]) #each probe point signal in a row
           ...
           self.organ_of_corti_acceleration[:,j]=self.passive[:]
            ...
            #  k=0
             # for i in self.probe_points:
              #  self.Vsolution[k,j]=self.Vtmp[i] 
               # self.Ysolution[k,j]=self.Ytmp[i]
                #self.organ_of_corti_acceleration[k,j]=self.passive[i]
                #k=k+1
