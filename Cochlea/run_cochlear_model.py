...
def solve_one_cochlea(model):
    ...
    #model needs to be init here because if not map crash
    model[0].init_model(model[1], Oversampling * Fs, sectionsNo, probe_points,
                        Zweig_irregularities=model[2], sheraPo=sheraPo,
                        subject=subjectNo)
    model[0].solve()
    return [model[0].Vsolution, model[0].Ysolution, model[0].oto_emission,
            model[0].stim[0:len(model[0].oto_emission)], model[0].f_resonance]
...
cochlear_list = [[main_loop_update.cochlea_model(), sig[i], irr_on[0][i]]
                 for i in range(channels)]
...
sio.savemat("outputPT1_update.mat",
