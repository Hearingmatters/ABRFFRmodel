...
def TLsolver(t,y,model): #y''=dv/dt y'=v
    n=model.n+1
    #F0=model.interplPoint1+(model.interplPoint2-model.interplPoint1)*(t-model.lastT)/model.dt #linear interpolatio, can be improved if needed (e.g. blinear)!
    ...
    cminusb=c-b
    F0=b + frac * (cminusb - 0.1666667* (1.-frac) * ((d - a - 3.0 * cminusb) * frac + (d + 2.0*a - 3.0*b))) #cubic spline interpolation (FS needs to be 3*BW of signal)
    model.Vtmp=y[0:n]
    model.Ytmp=y[n:2*n]
    tmpdev=model.Dev[0:n]
    #model.Dev[0:n]=model.Dev[0:n]+frac
    #libtrisolv.delay_line(model.Ybuffer_pointer,model.Zrp_pointer,model.Zrp1_pointer,model.Zrp2_pointer,model.Zrp3_pointer,model.Dev_pointer,model.YZweig_pointer,ctypes.c_int(model.YbufferLgt),ctypes.c_int(n))
    ...
    Vderivative=(model.Qsol-model.g)   #dt out not included here, the solver handles automatically the integration
...
#function to intitialize all the parameters        
    def init_model(self,stim,samplerate,sections,probe_freq,sheraPo,compression_slope=0.4,Zweig_irregularities=1,non_linearity_type="vel",KneeVar=1.,low_freq_irregularities=1,subject=1):
    ...
    def calculate_g(self): #same as in fortran
        ...
        self.passive=-1*(dtot[0:n]*self.Vtmp+self.SheraRho*self.YZweig*self.omega2)
    ...
    def polecalculation(self): #TODO
        ...
        if(self.use_Zweig):
            RthY1=self.Yknee1*Rth_norm #Normalized RTH, so save a bit of computation
	    ...
	    if(self.non_linearity==1): #To check
		    ...
		    Theta0=np.arctan(((self.PoleE-PoleS)/((Yknee2F/Yknee1F)-1.)))
		    Theta=Theta0/2.
		    ...
		    Theta0=np.arctan(((self.PoleE-PoleS)*factor)/((RthV2/RthV1)-1.))
		    Theta=Theta0/2.
    ...
    def solve(self):
        ...
        self.Vsolution=np.zeros([len(self.x),length]) #each probe point signal in a row
