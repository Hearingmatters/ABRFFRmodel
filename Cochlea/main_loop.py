...
def TLsolver(t, y, model):  # y''=dv/dt y'=v
    n = model.n + 1
    # F0=model.interplPoint1+(model.interplPoint2-model.interplPoint1)*
    #    (t-model.lastT)/model.dt
    # #linear interpolatio, can be improved if needed (e.g. blinear)!
    ...
    F0 = b + frac * \
        (cminusb - 0.1666667 * (1. - frac) *
         ((d - a - 3.0 * cminusb) * frac + (d + 2.0 * a - 3.0 * b)))
                     # cubic spline interpolation (FS needs to be 3*BW of
                     # signal)
    model.Vtmp = y[0:n]
    model.Ytmp = y[n:2 * n]
    tmpdev = model.Dev[0:n]
    # model.Dev[0:n]=model.Dev[0:n]+frac
    # libtrisolv.delay_line(model.Ybuffer_pointer,
    #                       model.Zrp_pointer, model.Zrp1_pointer,
    #                       model.Zrp2_pointer, model.Zrp3_pointer,
    #                       model.Dev_pointer, model.YZweig_pointer,
    #                       ctypes.c_int(model.YbufferLgt),ctypes.c_int(n))
    # model.Dev[0:n]=tmpdev[0:n]
    model.calculate_g()
    model.calculate_right(F0)
    #compute q
    libtrisolv.solve_tridiagonal(ctypes.byref(model.tridata),
                                 model.r_pointer, model.Qpointer,
                                 ctypes.c_int(n))
    #ME equation
    zero_val = (model.RK4_0 * model.Qsol[
                0] + model.RK4G_0 * (model.g[0] + model.p0x * F0))
    #dt out not included here, the solver handles automatically the integration
...
# function to intitialize all the parameters
    def init_model(self, stim, samplerate, sections, probe_freq, sheraPo,
        ...
        b, a = signal.butter(
            1, [100. / samplerate, 3000. / samplerate], 'bandpass')
    ...
    def SetDampingAndStiffness(self):
        ...
        self.probe_points = np.sort(self.probe_points)
        self.probe_points = np.array(self.probe_points, dtype=int)
        self.probe_points = np.flipud(self.probe_points)
    ...
    def calculate_g(self):  # same as in fortran
        ...
        self.passive = -1 * \
            (dtot[0:n] * self.Vtmp + self.SheraRho * self.YZweig * self.omega2)
    ...
    def ZweigImpedance(self):
        ...
        libtrisolv.delay_line(self.Ybuffer_pointer, self.Zrp_pointer,
                              self.Zrp1_pointer, self.Zrp2_pointer,
                              self.Zrp3_pointer, self.Dev_pointer,
                              self.YZweig_pointer,
                              ctypes.c_int(self.YbufferLgt),
                              ctypes.c_int(self.n + 1))
    ...
    def polecalculation(self):  # TODO
        # non-linearities  can be Wrapped from C for an additional boost,
        # but must be done carefully (efficient memory allocation
        # print self.SheraPo
        factor = 100.
        lf_limit = self.ctr
        n = self.n + 1
        ...
        # print self.use_Zweig
        if(self.use_Zweig):
            #Normalized RTH, so save a bit of computation
            RthY1 = self.Yknee1 * Rth_norm
            RthY2 = self.Yknee2 * Rth_norm
            RthV1 = self.Vknee1 * Rth_norm
            RthV2 = self.Vknee2 * Rth_norm

            Rndm = self.IrrPct * Rth / 2.
            PoleS = (1 + Rndm) * self.SheraPo
            # print(PoleS)
            # print self.Yknee1

            RthY1[lf_limit:n] = self.Yknee1
            RthY2[lf_limit:n] = self.Yknee2
            RthV1[lf_limit:n] = self.Vknee1
            RthV2[lf_limit:n] = self.Vknee2
            PoleS[lf_limit:n] = self.SheraPo[lf_limit:n]
            # PoleS[lf_limit:n]=0.061
            # print 'good'
            if(self.non_linearity == 1):  # To check
                # non-linearity DISP cost about three times more than in
                # fortran
                Yknee1CST = RthY1 * self.omega[self.onek]
                Yknee2CST = RthY2 * self.omega[self.onek]
                Yknee1F = Yknee1CST / self.omega
                Yknee2F = Yknee2CST / self.omega
                Yvect = np.abs(self.Ytmp / Yknee1F)
                Theta0 = np.arctan(
                    ((self.PoleE - PoleS) / ((Yknee2F / Yknee1F) - 1.)))
                Theta = Theta0 / 2.
                ...
                Sfoc = PoleS * factor * (Yknee2F / Yknee1F)
                # In original implementation Se=1./Se. replace division with
                # multiplication and save some microseconds
                Se = sin_Theta
                Sb = Sfoc * Se
                ...
                self.SheraP = PoleS + Sy / factor

            elif(self.non_linearity == 2):  # non-linearity VEL
                Vvect = np.abs(self.Vtmp) / RthV1
                Theta0 = np.arctan(
                    ((self.PoleE - PoleS) * factor) / ((RthV2 / RthV1) - 1.))
                Theta = Theta0 / 2.
                Sfoc = (PoleS * factor) / (RthV2 / RthV1)
            ...
            # as for loop. fmin instead of minimum, to be sure that non Nan
    ...
    def solve(self):
        ...
        self.Vsolution = np.zeros([len(self.x), length])
        self.Ysolution = np.zeros([len(self.x), length])
        self.oto_emission = np.zeros(length)
        self.time_axis = np.linspace(0, time_length, length)
        # constructor to ode
        # set integrator ('dopri5')=RK45 ('vode',method='adams')=explicit(no
        # more the fastest one, DO NOT WORK FOR PARALLEL CALL)
        # ('vode',method='bdf')=implicit
        #max_step=dt force to evaluate the input for every sample
        r = ode(TLsolver).set_integrator('dopri5', rtol=0.01, max_step=self.dt)
        r.set_f_params(self)
        r.set_initial_value(
        ...
        while(j < length):
            ...
            self.calculate_g()
           # if(j<np.size(self.Vsolution)):  #store displacement and velocity
           # for probe points, included in r.y
        ...
        samplerate = self.fs
        b, a = signal.butter(
            1, [600. / samplerate, 3000. / samplerate], 'bandpass')
